...
while kill -0 "$child_pid" 2>/dev/null; do
    ...
    echo "$script_name: ${percent}% complete"
    sleep $heartbeat
done
wait "$child_pid"
exit_code=$?
if [ "$exit_code" -eq 0 ]; then
    echo "$script_name: 100% complete"
    if [ "$level_uc" = "VERBOSE" ]; then
...
print_startup_info() {
    ...
    for dir in "${script_dirs[@]}"; do
        if [ -d "$dir" ]; then
            local count
            count=$(find "$dir" -type f -executable -name "*.sh" | wc -l | tr -d ' ')
            if [ "$count" != "0" ]; then
        ...
        if [ -d "$dir" ]; then
            ...
            find "$dir" -type f -executable -name "*.sh" | while read -r script; do
                ...
                if [ "${CONTAINER_USER:-appuser}" = "root" ]; then
                    ...
                    if su-exec ${CONTAINER_USER:-appuser} /usr/local/bin/run_script.sh "$script" "$script_name"; then
                        log_info "✓ $script_name completed successfully"
                    else
                        log_error "✗ $script_name failed with exit code $?"
                        exit 1
        ...
        for dir in "${script_dirs[@]}"; do
            if [ -d "$dir" ]; then
                ...
                find "$dir" -type f -executable -name "*.sh" | while read -r script; do
                    ...
                    if [ "${CONTAINER_USER:-appuser}" != "root" ]; then
                        echo "$cron_schedule su-exec ${CONTAINER_USER:-appuser} /bin/sh -c '/usr/local/bin/run_script.sh $script $script_name >> /proc/1/fd/1 2>&1'" >> /tmp/crontabs/root
                    else
                        echo "$cron_schedule /usr/local/bin/run_script.sh $script $script_name >> /proc/1/fd/1 2>&1" >> /tmp/crontabs/root
                    fi
                done
            fi
        done
